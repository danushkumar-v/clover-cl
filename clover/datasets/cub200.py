"""CUB-200-2011 dataset wrapper for CLOVER.

Download: https://drive.google.com/file/d/1XbUpnWpJPnItt5zQ6sHJnsjPncnNLvWb
Expected layout after extraction:
    <root>/cub/train/<class_id>/...
    <root>/cub/test/<class_id>/...
"""
...
_DOWNLOAD_URL = "https://drive.google.com/file/d/1XbUpnWpJPnItt5zQ6sHJnsjPncnNLvWb"
...
class CUB200Dataset(CLDataset):
    """CUB-200-2011 wrapper (200 bird species).

    Args:
        root: Parent directory containing the ``cub/`` subdirectory.
    ...
    Raises:
        FileNotFoundError: If the dataset directory is not found.  Download
            the archive from ``https://drive.google.com/file/d/1XbUpnWpJPnItt5zQ6sHJnsjPncnNLvWb``
            and extract it so that ``<root>/cub/train/`` exists.
    """
    ...
    def __init__(
        ...
        data_dir = f"{root}/cub/{split}"
        try:
            ds = datasets.ImageFolder(data_dir)
        except FileNotFoundError:
            raise FileNotFoundError(
                f"CUB-200 data not found at {data_dir!r}. "
                f"Download from {_DOWNLOAD_URL} and extract so that "
                f"'{root}/cub/train/' exists."
            )
