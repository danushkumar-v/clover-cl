...
``label_space`` (LabelSpaceView) is added in P2, once ``methods/losses.py``
exists to consume it. This phase keeps ``Experience`` pure/serializable —
unlike v1, it does not hold a live ``torch.utils.data.Dataset`` (that
required a real, loaded dataset and coupled ``clover/core`` to torch);
wrapping ``image_indices`` into an actual per-experience Dataset is a P3/
dataset-layer concern once real data exists.
"""
